...
##Front End
upper_part = Frame(root,padx=10,pady=10)
Label(upper_part,text = "write assembly instructions").pack(side=LEFT,fill=Y,expand=1)#pack
ask_open = Button(upper_part,text = 'or import from file ..')
ask_open.pack(side=LEFT,fill=Y)#pack
ask_open.bind("<Button-1>",ask_open_file)
upper_part.pack(expand=1)
#upper_part_done
mid_part = Frame(root)
entry = Text(mid_part,width=40,height=17)##height
...
#mid_part_is_over
lower_part = Frame(root)
add = Button(lower_part,text = "CONVERT!")
is_added= BooleanVar()
